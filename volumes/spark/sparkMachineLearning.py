...
from pyspark.sql.functions import (max, mean, monotonically_increasing_id,
                                   stddev, stddev_pop)
...
def calcDistance(point, clusterCenter, clusterCenters):
    ...
    dist = sqrt((cluster_x - point_x)**2 + (cluster_y - point_y)
                ** 2 + (cluster_z - point_z)**2)
    ...
    for row in data:
        ...
        for i in range(0, 3):
            ...
            if SCALING_FLAG:
                ...
                # scale the data.
                sumOfBytes_scaled = (
                    sumOfBytes - sumOfBytes_mean)/(sumOfBytes_stddev)
                uniqDstIPs_scaled = (
                    uniqDstIPs - uniqDstIPs_mean)/(uniqDstIPs_stddev)
                uniqDstPorts_scaled = (
                    uniqDstPorts - uniqDstPorts_mean)/(uniqDstPorts_stddev)
            else:
                # Don't scale.
...
def readDataFromES():
    ...
    results_gen = elasticsearch.helpers.scan(
        es, index='netflowrepo', doc_type='entry', query={"query": {"match_all": {}}})
    ...
    tmp1 = np.concatenate(
        (np_Bytes.reshape((-1, 1)), np_DstIPs.reshape((-1, 1))), axis=1)
    ...
    if SCALING_FLAG:
        ...
        predictions = predictions.rdd.map(extract).toDF(
            ["id", "prediction", "scaledFeatures", "sumOfBytes", "uniqDstIPs", "uniqDstPorts"])
    ...
    printDebugMsg("sumOfBytes_Orig_stddev = {0}".format(
        sumOfBytes_Orig_stddev))
    printDebugMsg("uniqDstIPs_Orig_stddev = {0}".format(
        uniqDstIPs_Orig_stddev))
    printDebugMsg("uniqDstPorts_Orig_stddev = {0}".format(
    ...
    printDebugMsg("uniqDstPorts_All_stddev = {0}".format(
    ...
    printDebugMsg("uniqDstPorts_Orig_mean = {0}".format(
        uniqDstPorts_Orig_mean))
    ...
    upperThreshold_0_Bytes = sumOfBytes_0_max + \
        NUM_STDDEV_ABOVE * sumOfBytes_0_stddev
    printDebugMsg("upperThreshold_0_Bytes = {0}".format(
        upperThreshold_0_Bytes))

    upperThreshold_1_Bytes = sumOfBytes_1_max + \
        NUM_STDDEV_ABOVE * sumOfBytes_1_stddev
    printDebugMsg("upperThreshold_1_Bytes = {0}".format(
        upperThreshold_1_Bytes))

    upperThreshold_2_Bytes = sumOfBytes_2_max + \
        NUM_STDDEV_ABOVE * sumOfBytes_2_stddev
    printDebugMsg("upperThreshold_2_Bytes = {0}".format(
        upperThreshold_2_Bytes))

    upperThreshold_0_DstIPs = uniqDstIPs_0_max + \
        NUM_STDDEV_ABOVE * uniqDstIPs_0_stddev
    printDebugMsg("upperThreshold_0_DstIPs = {0}".format(
        upperThreshold_0_DstIPs))

    upperThreshold_1_DstIPs = uniqDstIPs_1_max + \
        NUM_STDDEV_ABOVE * uniqDstIPs_1_stddev
    printDebugMsg("upperThreshold_1_DstIPs = {0}".format(
        upperThreshold_1_DstIPs))

    upperThreshold_2_DstIPs = uniqDstIPs_2_max + \
        NUM_STDDEV_ABOVE * uniqDstIPs_2_stddev
    printDebugMsg("upperThreshold_2_DstIPs = {0}".format(
        upperThreshold_2_DstIPs))

    upperThreshold_0_DstPorts = uniqDstPorts_0_max + \
        NUM_STDDEV_ABOVE * uniqDstPorts_0_stddev
    printDebugMsg("upperThreshold_0_DstPorts = {0}".format(
        upperThreshold_0_DstPorts))

    upperThreshold_1_DstPorts = uniqDstPorts_1_max + \
        NUM_STDDEV_ABOVE * uniqDstPorts_1_stddev
    printDebugMsg("upperThreshold_1_DstPorts = {0}".format(
        upperThreshold_1_DstPorts))

    upperThreshold_2_DstPorts = uniqDstPorts_2_max + \
        NUM_STDDEV_ABOVE * uniqDstPorts_2_stddev
    printDebugMsg("upperThreshold_2_DstPorts = {0}".format(
        upperThreshold_2_DstPorts))

    # Combined upper threshold is pythagorean in 3 dimensions.
    # s^2 = x^2 + y^2 + z^2
    threshold_0 = sqrt(upperThreshold_0_Bytes**2 +
                       upperThreshold_0_DstIPs**2 + upperThreshold_0_DstPorts**2)
    threshold_1 = sqrt(upperThreshold_1_Bytes**2 +
                       upperThreshold_1_DstIPs**2 + upperThreshold_1_DstPorts**2)
    threshold_2 = sqrt(upperThreshold_2_Bytes**2 +
                       upperThreshold_2_DstIPs**2 + upperThreshold_2_DstPorts**2)
    ...
    uniqDstIPs_2 = (uniqDstIPs_2_max, uniqDstIPs_Orig_mean,
                    uniqDstIPs_Orig_stddev)
    uniqDstPorts_0 = (uniqDstPorts_0_max,
                      uniqDstPorts_Orig_mean, uniqDstPorts_Orig_stddev)
    uniqDstPorts_1 = (uniqDstPorts_1_max,
                      uniqDstPorts_Orig_mean, uniqDstPorts_Orig_stddev)
    uniqDstPorts_2 = (uniqDstPorts_2_max,
                      uniqDstPorts_Orig_mean, uniqDstPorts_Orig_stddev)
...
def sendToONOS(anomalyIP):
    ...
    hosts = requests.get(
        'http://10.28.34.2:8181/onos/v1/hosts', auth=('onos', 'rocks'))
    ...
    blockData = {
        ...
        "selector": {
            "criteria": [
                ...
                {
                        "type": "IPV4_SRC",
                        # Must include subnet mask.
                        "ip": "{0}/32".format(anomalyIP)
                    }
            ]
        }
    }

    urlToPost = "http://10.28.34.2:8181/onos/v1/flows/{0}?appId=org.onosproject.fwd".format(
        ipToSwitchMap[anomalyIP])
    print("urlToPost = {0}".format(urlToPost))
    resp = requests.post(urlToPost, data=json.dumps(
        blockData), auth=('onos', 'rocks'))
...
if __name__ == "__main__":
    ...
    # the number indicates how many seconds each batch lasts.
    ssc = StreamingContext(sc, 60)

    # Creates an input stream that pulls events from Kafka.
    kvs = KafkaUtils.createStream(
        ssc, "louis:2181", "spark-streaming-consumer", {"NETFLOW": 1})
    parsed = kvs.map(lambda x: json.loads(x[1]))

    # Get only elements that are needed and rename to make it clear.
    netflow_dict = parsed.map(lambda x: ({'srcAddr': x['srcaddr_s'], 'srcPort': x['srcport'], 'dstAddr': x['dstaddr_s'], 'dstPort': x['dstport'],
                                          'tcpFlags': x['tcp_flags'], 'protocol': x['proto'], 'timestampStart': x['first'], 'timestampEnd': x['last'], 'numBytes': x['dOctets'], 'numFlows': x['count']}))
    # # # netflow_dict.pprint()

    # Get Sum of Flows sent from Source IP in window.
    sumOfFlows = netflow_dict.filter(lambda e: "srcAddr" in e).map(
        lambda e: (e["srcAddr"], e["numFlows"])).reduceByKey(lambda x, y: x + y)

    # Get sum of Bytes sent from Source IP in window.
    sumOfBytes = netflow_dict.filter(lambda e: "srcAddr" in e).map(
        lambda e: (e["srcAddr"], e["numBytes"])).reduceByKey(lambda x, y: x + y)
    ...
    uniqDstIPs = netflow_dict.filter(lambda e: "srcAddr" in e).map(lambda e: (
        e["srcAddr"], e["dstAddr"])).countByValue().map(lambda e: e[0][0]).countByValue()
    ...
    uniqDstPorts = netflow_dict.filter(lambda e: "srcAddr" in e).map(lambda e: (
        e["srcAddr"], e["dstPort"])).countByValue().map(lambda e: e[0][0]).countByValue()
    ...
    joined = join2.map(lambda e: (
        {"srcAddr": e[0], "sumOfBytes": e[1][0][0], "uniqDstIPs": e[1][0][1], "uniqDstPorts": e[1][1]}))
