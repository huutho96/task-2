...
CONTROLLER_IP = "10.28.34.2"
CONTROLLER_PORT = 6633

NETFLOW_TARGETS = ["10.28.34.4:9995", "10.28.34.4:9997"]
...
def runMyTopo():
    ...
    for s in net.switches:
        ...
        netflowCommand = "ovs-vsctl -- set bridge {0} netflow=@nf -- --id=@nf create Netflow targets=[\"{1}\",\"{2}\"] active-timeout=60".format(
            s, NETFLOW_TARGETS[0], NETFLOW_TARGETS[1])

        printDebugMsg("NETFLOW command is: {0}".format(netflowCommand))
        printDebugMsg("netflowCommand.split() command is: {0}".format(
            netflowCommand.split()))
    ...
    if not TESTMODE:
        ...
        for h in net.hosts:
            printDebugMsg(
                "Starting Client and Server on {0}. IP is {1}".format(h, h.IP()))
